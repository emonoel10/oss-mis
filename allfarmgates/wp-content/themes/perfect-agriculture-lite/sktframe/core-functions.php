<?php
/**
 * The Core Functions for Perfect Agriculture Lite
 *
...
// [readmore align="center" button="View All" links="#"]
function readmore_func( $atts) {
	...
	$rrow = '<div class="view-all-btn" style="text-align:'.$align.'"><a href="'.$links.'" '.( ($target!='') ? 'target='.$target.'' : '' ).'>'.$button.'</a></div>';
    return $rrow;
}
   ...
   extract( shortcode_atts( array(
	...
	if ( have_posts() ) :
		while ( have_posts() ) : the_post();
			...
			$postoutput .= '<li><h5><a href="'.get_the_permalink().'">'.get_the_title().'</a></h5></li>';
		endwhile;
	endif;
	wp_reset_query();
	$postoutput .= '</ul></div>';
