...
function mesmerize_print_buttons_list($setting, $default = array())
{
    ...
    foreach ($buttons as $index => $button) {
        ...
        if (is_customize_preview()) {
            ...
            $btn_string = '<a class="%4$s" target="%3$s" href="%1$s" ' . $mod_attr . ' ><span class="arrow-left-after">%2$s</span></a>';
            printf($btn_string, esc_url($url), wp_kses_post($title), esc_attr($target), esc_attr($class));
        } else {
            printf('<a class="%4$s" target="%3$s" href="%1$s"><span class="arrow-left-after">%2$s</span></a>', esc_url($url), wp_kses_post($title), esc_attr($target), esc_attr($class));
        }
    }
}
